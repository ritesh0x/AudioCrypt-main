...
class AudioCrypt:
    ...
    def encode(self, input_audio: str, output_audio: str, message: str, password: str, progress_callback=None):
        ...
        try:
            ...
            print(f"DEBUG: Total samples read for encoding: {len(samples)}")
            ...
            print(f"DEBUG: Modified samples length: {len(modified_samples)}")
    ...
    def decode(self, input_audio: str, password: str, progress_callback=None) -> tuple:
        ...
        try:
            ...
            # Debug: print length of samples
            print(f"DEBUG: Total samples read: {len(samples)}")

            # Extract magic header
            header_data = self._extract_data_lsb(samples, len(self.MAGIC_HEADER), progress_callback)
            print(f"DEBUG: Extracted header: {header_data}")
            ...
            print(f"DEBUG: Extracted IV: {iv}")

            offset += self.IV_SIZE * 8
            length_samples = samples[offset:offset + 4 * 8]
            encrypted_length_bytes = self._extract_data_lsb(length_samples, 4)
            encrypted_length = struct.unpack('<I', encrypted_length_bytes)[0]
            print(f"DEBUG: Extracted encrypted length: {encrypted_length}")
            ...
            print(f"DEBUG: Extracted encrypted message length: {len(encrypted_message)}")
